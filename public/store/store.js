...
export const store = new Vuex.Store({
  ...
  mutations: {
    ...
    DeleteCardTodatabase(state, card) {
      ...
      axios.delete("/api/delete/card/" + card.id).then(response => {
        console.log(response.data);
      });
    },
    UpdateCardStatusTodatabase(state, card) {
      axios
        .put("/api/update/card/status/" + card.id, {
          data: card
        })
        .then(response => {
          console.log(response.data);
  ...
  actions: {
    ...
    fetchCards(state, params) {
      axios
        .get("/api/cards/" + params.limit + "/" + params.offset)
        .then(cards => {
